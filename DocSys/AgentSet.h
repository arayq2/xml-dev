...
namespace DocSys
{
    ...
    struct LoaderMethods
    {
        static void add_title( Client& client, std::string const& title )
        {
            client.add_title( title );
        }

        static void add_item( Client& client, std::string const& title, std::string const& xpath )
        {
            client.add_item( title, xpath );
        }
    };

    template<typename Client, typename Methods = LoaderMethods<Client> >
    class SetLoader
    {
    public:
        ...
        , index_(1)
        {
            Methods::add_title( client_, parser_.title() );
        }
    ...
    private:
        ...
        {
            auto    _lambda([&]( std::string const& title, std::string const& path ) -> void
            {
    ...
    {
    public:
        friend class LoaderMethods<AgentSet<Agent> >;
    ...
    private:
        ...
        void add_item( std::string const& title, std::string const& path )
        {
            header_.push_back( title );
